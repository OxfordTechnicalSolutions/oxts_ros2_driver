...
BOOST_AUTO_TEST_CASE(hprToQuaternion) {
    BOOST_CHECK(approxEqual(
        ::hprToQuaternion(0, 0, 0),
        (std::vector<double>{0, 0, 0, 1})
    ));
    BOOST_CHECK(approxEqual(
        ::hprToQuaternion(360, 360, 360),
        (std::vector<double>{0, 0, 0, -1})
    ));
    BOOST_CHECK(approxEqual(
        ::hprToQuaternion(-90, -180, -270),
...
BOOST_AUTO_TEST_CASE(geodeticToEcef) {
    BOOST_CHECK(approxEqual(
        ::geodeticToEcef(0, 0, 0),
        Point::Cart{6378137, 0, 0}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEcef(44, 11, 9999),
        Point::Cart{4518105.0405, 878230.6545, 4415037.5015}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEcef(-3.14, -1.23, 55),
        Point::Cart{6367212.7422, -136709.4584, -347035.9514}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEcef(9999, 9999, 9999),
...
BOOST_AUTO_TEST_CASE(ecefToGeodetic) {
    BOOST_CHECK(approxEqual(
        ::ecefToGeodetic(0, 0, 0),
        Point::Geodetic{0, 0, -6378137}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToGeodetic(44, 11, 9999),
        Point::Geodetic{-89.528, 14.0362, -6394086.5905}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToGeodetic(-3.14, -1.23, 55),
        Point::Geodetic{-2.7554, -158.6087, -6378182.9612}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToGeodetic(9999, 9999, 9999),
...
BOOST_AUTO_TEST_CASE(ecefToEnu) {
    BOOST_CHECK(approxEqual(
        ::ecefToEnu(Point::Cart{1, 2, 3}, 20, 50, 10),
        Point::Cart{0.5195, 13730.219, -6375646.1017}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToEnu(Point::Cart{999, 999, 999}, 20, 50, 10),
        Point::Cart{-123.1336, 14185.5296, -6373984.9481}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToEnu(Point::Cart{45, 45, 30}, -3.14, -1.23, 55),
        Point::Cart{45.9556, -2302.9496, -6378085.6303}
    ));
    BOOST_CHECK(approxEqual(
        ::ecefToEnu(Point::Cart{45, 45, 30}, 9999, 9999, 9999),
...
BOOST_AUTO_TEST_CASE(enuToLrf) {
    ...
    BOOST_CHECK(approxEqual(
        ::enuToLrf(100, 100, 100, 180),
        Point::Cart{20.2696, -139.9613, 100}
    ));
    BOOST_CHECK(approxEqual(
        ::enuToLrf(450, 340, 230, -90),
...
BOOST_AUTO_TEST_CASE(geodeticToEnu) {
    BOOST_CHECK(approxEqual(
        ::geodeticToEnu(0, 0, 0, 0, 0, 0),
        Point::Cart{0, 0, 0}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEnu(40, 50, 60, 44, 11, 9999),
        Point::Cart{3079109.5854, 313497.8013, -809786.2658}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEnu(-100, -100, -100, -3.14, -1.23, 55),
        Point::Cart{1098156.3573, -6243102.4693, -5866100.6648}
    ));
    BOOST_CHECK(approxEqual(
        ::geodeticToEnu(9999, 9999, 9999, 9999, 9999, 9999),
        Point::Cart{0, 0, 0}
    ));
}
