...
struct Fixture {
    Fixture() {
        ...
        topicPrefix = driver->get_parameter("topic_prefix").get_value<std::string>();
    }
...
template<typename Message> void waitForMessage(
    std::string topic,
    ...
    auto _ = ins->create_subscription<Message>(topic, 1, [&gotMsg](typename Message::SharedPtr) {
        gotMsg.unlock();
    });

    BOOST_CHECK_MESSAGE(
        gotMsg.try_lock_for(timeout),
        std::string{"receiving a message on the topic \""} + topic + '"'
    );
}
