...
BOOST_AUTO_TEST_CASE(getVat) {
    ...
    BOOST_CHECK(approxEqual(::getVat(&nrx), (tf2::Quaternion{-0.5, -0.5, 0.5, 0.5})));
}
...
BOOST_AUTO_TEST_CASE(getVehRPY) {
    ...
    nrx.mPitch = 180;
    nrx.mHeading = 270;
    BOOST_CHECK(approxEqual(::getVehRPY(&nrx), (tf2::Quaternion{0.707107, 0, 0, 0.707107})));
}
...
geometry_msgs::msg::PoseStamped makePose(int ox, int oy, int oz, int px, int py, int pz) {
...
BOOST_AUTO_TEST_CASE(path) {
    std::vector<geometry_msgs::msg::PoseStamped> poses{
        makePose(1, 2, 3, 4, 5, 6),
        makePose(7, 8, 9, 10, 11, 12),
        makePose(13, 14, 15, 16, 17, 18),
        makePose(19, 20, 21, 22, 23, 24),
        makePose(25, 26, 27, 28, 29, 30),
        makePose(31, 32, 33, 34, 35, 36),
