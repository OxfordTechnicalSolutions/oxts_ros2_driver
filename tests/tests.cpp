...
bool approxEqual(const std::vector<double>& l, const std::vector<double>& r, double delta) {
    if (l.size() != r.size())
        return false;
...
bool approxEqual(const Point::Geodetic& l, const Point::Geodetic& r, double delta) {
    return approxEqual(l.lat(), r.lat(), delta)
        && approxEqual(l.lon(), r.lon(), delta)
        && approxEqual(l.alt(), r.alt(), delta);
}

bool approxEqual(const tf2::Quaternion& l, const tf2::Quaternion& r, double delta) {
    return approxEqual(l.x(), r.x(), delta)
        && approxEqual(l.y(), r.y(), delta)
        && approxEqual(l.z(), r.z(), delta);
}
